...
const averageWordLength = (array) => {
  ...
  array.forEach((element) => (averageWordLength += element.length));
  if (array.length === 0) {
    return null;
  } else {
...
const howManyTimes = (array, word) => {
  return array.filter((currentWord) => word === currentWord).length;
};
...
const greatestProduct = (matrix) => {
  let theGreatestProduct = 1;

  for (let i = 0; i < matrix.length; i++) {
    for (let j = 0; j < matrix[i].length - 4; j++) {
      if (theGreatestProduct < productNumbers(matrix[i].slice(j, 4))) {
        theGreatestProduct = productNumbers(matrix[i].slice(j, 4));
      }
    }
  }
  ...
  for (let i = 0; i < matrix.length - 4; i++) {
    for (let j = 0; j < matrix[i].length; j++) {
      if (
        ...
        theGreatestProduct = productNumbers([
          matrix[i][j],
          matrix[i + 1][j],
          matrix[i + 2][j],
          matrix[i + 3][j]
